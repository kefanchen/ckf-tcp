		...
		uint32_t cur_ts, uint8_t *payload, uint32_t seq, int payloadlen)
{
	...
	int ret;
	uint32_t seq_per_pkt = 0;
	uint32_t num_in_4 = 0;
	struct fragment_ctx* iter = 0;
	struct fragment_ctx* red_fctx = 0;
	...
	//ckf mod ,use the redundacy here
	
	if(payloadlen != PKT_SIZE || (seq - rcvvar->rcvbuf->init_seq )%PKT_SIZE != 0){
		printf("ckf payload is not the PKT_SIZE in processpayload\n");
		return FALSE;
	}
	seq_per_pkt =(uint32_t) (seq - rcvvar->rcvbuf->init_seq)/PKT_SIZE;
	num_in_4 = seq_per_pkt % 4;
	if(num_in_4 == 3)//this payload is a redundancy
	{
		red_fctx = NULL;	
		//find the fctx corresponding to the redundancy
		for(iter = rcvvar->rcvbuf->fctx;iter != NULL; iter = iter->next)
		{
			if(seq - 3*PKT_SIZE <= iter->seq && seq - PKT_SIZE >= iter->seq)
			{
				red_fctx = iter;
				break;
			}
		...
		//only handle missng exactly one pkt in 3
		if(red_fctx)
		{
			if(red_fctx->seq == seq -PKT_SIZE * 3)
			{
				if(red_fctx->len == 2*PKT_SIZE)//NO.2 pkt miss
				{ 
					memset(recovery,0,PKT_SIZE);
					
					//use NO.0 pkt
					for(i=0;i<PKT_SIZE;i++)
						recovery[i] = recovery[i] ^
						rcvvar->rcvbuf->head[red_fctx->seq - rcvvar->rcvbuf->head_seq + i];
					
					//use NO.1 pkt
					for(i=0; i<PKT_SIZE;i++)
						recovery[i] = recovery[i] ^ 
						rcvvar->rcvbuf->head[red_fctx->seq + PKT_SIZE -rcvvar->rcvbuf->head_seq + i];
				...
				rcvvar->rcvbuf, recovery, (uint32_t)payloadlen,seq-PKT_SIZE );
				//ack
				EnqueueACK(mtcp, cur_stream, cur_ts, ACK_OPT_AGGREGATE);
				...
				{
					if(red_fctx->len == PKT_SIZE && (red_fctx->next != NULL)
					&&red_fctx->next->seq == seq - PKT_SIZE)//NO.1 pkt miss
					{
						memset(recovery,0,PKT_SIZE);
						...
						for(i=0; i<PKT_SIZE;i++)
							recovery[i] = recovery[i] ^ 
							rcvvar->rcvbuf->head[seq - PKT_SIZE - rcvvar->rcvbuf->head_seq + i];
					...
					//RBPut the NO.1 pkt		
					ret = RBPut(mtcp->rbm_rcv, 
					rcvvar->rcvbuf, recovery, (uint32_t)payloadlen,seq - 2*PKT_SIZE );
					EnqueueACK(mtcp, cur_stream, cur_ts, ACK_OPT_AGGREGATE);
			...
			{
				if(red_fctx->seq == seq - 2*PKT_SIZE && red_fctx->len == 2*PKT_SIZE)
				//NO.0 pkt miss
				{
					//use NO.1 pkt
					for(i=0; i<PKT_SIZE ;i++)
				...
				//RBPut the NO.0 pkt		
				ret = RBPut(mtcp->rbm_rcv, 
				rcvvar->rcvbuf, recovery, (uint32_t)payloadlen,seq-3*PKT_SIZE );
				EnqueueACK(mtcp, cur_stream, cur_ts, ACK_OPT_AGGREGATE);
	...
	else// the payload is no redundancy
	ret = RBPut(mtcp->rbm_rcv, 
	...
	//Though rcv payloadm , mergedlen does not increase, there are gap in buf
