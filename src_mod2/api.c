...
CloseStreamSocket(mctx_t mctx, int sockid)
{
	...
	{
		if(!(buf->num_in_4==0 && buf->pkt_frag ==0))//if not complete
		{
			to_add = (2 - buf->num_in_4)*PKT_SIZE + PKT_SIZE - buf->pkt_frag;
			data_add = (char*)calloc(to_add,sizeof(char));
			if(data_add)
			{
				SBUF_LOCK(&(cur_stream->sndvar->write_lock));
				add_len = SBPut(mtcp->rbm_snd,buf,data_add,to_add);
				if(add_len != to_add)
					printf("ckf mod add_len ! = to_add in mtcp_close!\n");
				
				free(data_add);
				data_add = 0;

				// add the redundancy
				add_len = SBPut(mtcp->rbm_snd,buf,buf->redundancy,PKT_SIZE);
				if(add_len != PKT_SIZE)
					printf("ckf mod redundancy add_len != PKT_SIZE in mtcp_close!\n");
