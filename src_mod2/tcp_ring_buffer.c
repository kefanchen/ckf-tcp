	   ...
	   void* data, uint32_t len, uint32_t cur_seq)
{
	...
	else //ckf mod ,skip the redundancy gap in buffer
	{
		if((buff->head_seq - buff->init_seq)% PKT_SIZE == 0)
		{
			seq_per_pkt =(uint32_t) (buff->head_seq - buff->init_seq)/PKT_SIZE;
			num_in_4 = seq_per_pkt % 4;
			if(num_in_4 == 3 &&( buff->fctx->seq - buff->head_seq== PKT_SIZE))
			{
				buff->head_seq =buff-> fctx->seq;
				buff->head_offset += PKT_SIZE;
